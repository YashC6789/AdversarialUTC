...
# mcg.py  (around line 175)

def to_nested_cpu(seq_list):
    # make sure every sub-tensor is on the CPU first
    seq_list = [t.to("cpu") for t in seq_list]
    # build the NestedTensor *explicitly* on the CPU
    return torch.nested.nested_tensor(seq_list, device="cpu")  # or as_nested_tensor
...
def cal_losses(
    ...
    nested_ids = to_nested_cpu(test_ids)
